...
foreach ($file in $total_files)
{
  ...
  if (!(Test-Path -Path "./logs/NLinear_p5/${file}_abl"))
  {
    New-Item -ItemType Directory -Path "./logs/NLinear_p5/${file}_abl"
  }
}
# seq_len=700
$model_name = "NLinear"
...
foreach ($file in $light_files)
{
  ...
  --num_workers 0 `
  --patience 5 `
  --itr 1 --batch_size $bs --learning_rate 0.0005 > logs/NLinear_p5/${file}_abl/$m'_'$model_name'_feature'$features'_'Etth1_$seq_len'_'$pred_len'_H'$H_order'_bs'$bs'_s'$seed.log
