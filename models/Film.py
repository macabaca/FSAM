...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        if configs.ab == 0:
            ...
            encoder_self_att = SpectralConv1d(in_channels=configs.d_model, out_channels=configs.d_model, seq_len=self.seq_len)
            decoder_self_att = SpectralConv1d(in_channels=configs.d_model, out_channels=configs.d_model, seq_len=self.seq_len//2+self.pred_len)
            ...
            encoder_self_att = FullAttention(False, configs.factor, attention_dropout=configs.dropout, output_attention=configs.output_attention)
            decoder_self_att = FullAttention(False, configs.factor, attention_dropout=configs.dropout, output_attention=configs.output_attention)
            decoder_cross_att = FullAttention(False, configs.factor, attention_dropout=configs.dropout, output_attention=configs.output_attention)
        elif configs.ab == 8:
            encoder_self_att = ProbAttention(False, configs.factor, attention_dropout=configs.dropout, output_attention=configs.output_attention)
            decoder_self_att = ProbAttention(False, configs.factor, attention_dropout=configs.dropout, output_attention=configs.output_attention)
            decoder_cross_att = ProbAttention(False, configs.factor, attention_dropout=configs.dropout, output_attention=configs.output_attention)
        elif configs.ab == 5:
            ...
            decoder_self_att = SpectralConvCross1d(in_channels=configs.d_model, out_channels=configs.d_model,
                                                    seq_len_q=self.seq_len//2+self.pred_len, seq_len_kv=self.seq_len//2+self.pred_len)
            decoder_cross_att = SpectralConvCross1d(in_channels=configs.d_model, out_channels=configs.d_model,
                                                    seq_len_q=self.seq_len//2+self.pred_len, seq_len_kv=self.seq_len)
        elif configs.ab == 6:
            encoder_self_att = SpectralConv1d(in_channels=configs.d_model, out_channels=configs.d_model, seq_len=self.seq_len, modes1=configs.modes1)
            ...
            decoder_cross_att = MWT_CZ1d_cross(in_channels=configs.d_model, out_channels=configs.d_model,seq_len_q=self.seq_len//2+self.pred_len, seq_len_kv=self.seq_len, modes1=configs.modes1, ich=configs.d_model,base=configs.base,activation=configs.cross_activation)
        elif config.ab == 8:
            encoder_self_att = SpectralConv1d_local(in_channels=configs.d_model, out_channels=configs.d_model, seq_len=self.seq_len, modes1=configs.modes1)
            decoder_self_att = SpectralConv1d_local(in_channels=configs.d_model, out_channels=configs.d_model, seq_len=self.seq_len//2+self.pred_len, modes1=configs.modes1)
            decoder_cross_att = SpectralConvCross1d_local(in_channels=configs.d_model, out_channels=configs.d_model,
                                                    seq_len_q=self.seq_len//2+self.pred_len, seq_len_kv=self.seq_len, modes1=configs.modes1)
