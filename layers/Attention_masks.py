...
class Mahalanobis_mask(nn.Module):
    ...
    def forward(self, X):
        ...
        # Probability that two channels are related in bernoulli
...
class GC_prob_mask(nn.Module):
    def __init__(self, mask_file):
        super(GC_prob_mask, self).__init__()
    ...
    def forward(self, X):
        ...
        mask = sample.unsqueeze(0).expand(B, -1, -1)
        mask = mask.unsqueeze(1)
        cnt = torch.sum(mask, dim=-1)
        return mask.to(X.device)

class Granger_Causality_mask(nn.Module):
    def __init__(self, mask_file, threshold):
        super(Granger_Causality_mask, self).__init__()
        self.mask = torch.load(mask_file)
    ...
    def forward(self, X):
        ...
        binary_tensor = (self.mask > self.threshold).float()
        expanded_mask = binary_tensor.unsqueeze(0).expand(B, -1, -1)
        expanded_mask = expanded_mask.unsqueeze(1)

        return expanded_mask.to(X.device)
